...
def get_token_auth_header():
    ...
    header_disassembly = header.split(' ')
    print(header_disassembly)
    if len(header_disassembly) != 2 or not header_disassembly:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header must be in the format'
            ' Bearer token'}, 401)

    elif header_disassembly[0].lower() != 'bearer':
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header must start with Bearer'}, 401)
    print(header_disassembly[1])

    return header_disassembly[1]
...
@TODO implement @requires_auth(permission) decorator method
    ...
    it should use the get_token_auth_header
    ...
    decoded payload to the decorated method /Done
'''
